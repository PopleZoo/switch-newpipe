...
- 홈 피드 무한 스크롤 (마지막 행 근처로 이동하면 다음 페이지 자동 로드)
- `ZR` 풀스크린 탭 메뉴 (전체 5개 탭 전환, 사이드바 숨김)
- 비동기 썸네일 로딩
...
- 홈 피드는 아래로 내려가며 계속 다음 페이지를 불러온다 (무한 스크롤)
- `ZR`로 풀스크린 탭 전환 메뉴 열기 (사이드바가 숨겨져 있어 탭 전환용)
- 실제 YouTube 응답 기반 검색
- 목록 화면에서 `A`로 즉시 재생
- `Y`로 상세 화면 진입
...
- progressive / UMP 경로의 시간 이동은 이미 받아둔 구간 안에서만 된다. 이동 가능한
  범위는 OSD 진행 바의 밝은 회색 구간으로 표시된다
- 라이브는 시간 이동이 안 된다
